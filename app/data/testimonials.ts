export const testimonials = [
  {
    id: 0,
    firstName: "Aaron",
    lastName: "White",
    quote: `"I made the switch to Charles and Bugra 
    during a very difficult time in my career, and 
    it’s one of the best decisions I’ve ever made. 
    From day one, they have gone above and beyond 
    not only as agents but as supporters of me and 
    my family. Their approach, honesty, and commitment 
    have given me peace and allowed me to stay in 
    the market I want to be! They’ve put my best 
    interests first and treated me like more than 
    just a client. I’m grateful for everything they’ve 
    done and continue to do for us!"`,
    image: "/images/aaron-white.png",
    alt: "Photo of SLASH basketball athlete Aaron White"
  },
  {
    id: 1,
    firstName: "Ryan",
    lastName: "Kelly",
    quote: `"When my close friend and agent decided to 
    change professions after my 10th year as a pro with 
    him, it wasn’t easy putting my trust in someone 
    new—especially overseas. After a recommendation 
    from Slash client James Michael McAdoo, I took a 
    meeting with Charles. He promised that Slash just 
    wanted to be additive and there for whatever I 
    needed at this point in my career. That was the 
    truth. Charles and Bugra have done nothing but care 
    for my family and me since signing with them. They 
    are on top of every detail- always supportive, 
    communicative, and wanting to help. Placing my 
    trust in them has set me up to finish my career in 
    a great place."`,
    image: "/images/ryan-kelly.png",
    alt: "Photo of SLASH basketball athlete Ryan Kelly"
  },
  {
    id: 2,
    firstName: "Isaiah",
    lastName: "Austin",
    quote: `"Charles is more than an agent to me. He’s 
    a friend and a brother. When I first signed with 
    him in 2017 he promised me a relationship that 
    would extend beyond the basketball player I was. He 
    told me he would be with me through the fire, no 
    matter what. In my career I have been cut, injured, 
    and given up on by many but Charles always had 
    faith in me. He constantly poured love into my ear 
    when I needed it most but also was not afraid to 
    tell me what it was that I was doing wrong in 
    situations as well. When people ask me about 
    Charles I always tell them about his leadership, he 
    leads by serving and those are always the best 
    leaders in the world, the ones who are not afraid 
    to fight with you. In my 5 years he helped me 
    develop as a man but also challenged me to develop 
    as a ball player. He’s the best in the game, second 
    to none. Slash VS Everybody!!!"`,
    image: "/images/isaiah-austin.jpeg",
    alt: "Photo of SLASH basketball athlete Isaiah Austin"
  },
  {
    id: 3,
    firstName: "Thomas",
    lastName: "Wimbush",
    quote: `"Since joining Slash I've been extremely 
    happy with my progress in my career path and more 
    importantly my relationship with Charles & Bugra. 
    We created a family type relationship that usually 
    you don’t get in this business and I want to thank 
    Slash management for welcoming me and my family."`,
    image: "/images/thomas-wimbush.jpeg",
    alt: "Photo of SLASH basketball athlete Thomas Wimbush"
  },
  {
    id: 4,
    firstName: "Brock",
    lastName: "Motum",
    quote: `"Moving to Slash with Charles and Buğra 
    has been hands down the best decision I have made 
    in my career. I was representing myself at the time 
    as I’d had enough of getting lost in the mix by 
    past agents. Since joining slash they have 
    presented opportunities I could’ve only dreamt of a 
    couple of years ago. I now think what could’ve been 
    if I had joined them earlier in my career. Beyond 
    grateful and changed my life. Thank you guys!"`,
    image: "/images/brock-motum.jpeg",
    alt: "Photo of SLASH basketball athlete Brock Motum"
  },
  {
    id: 5,
    firstName: "Nick",
    lastName: "Rakocevic",
    quote: `"Im so blessed & thankful to have made the 
    decision to sign with Charles & SLASH. Ever since 
    day one Charles has done whatever he could to make 
    sure I was happy with my career. Not only has he 
    helped my career on the court, but he has been 
    there for me when I needed him in my daily life. He 
    has become a true friend of mine & my brother. He 
    is a great agent and even a better person. I am 
    truly thankful to have him represent me & to call 
    him a friend! #SLASH."`,
    image: "/images/nick-rakocevic.jpeg",
    alt: "Photo of SLASH basketball athlete Nick Rakocevic"
  },
  {
    id: 6,
    firstName: "Coty",
    lastName: "Clarke",
    quote: `"During the prime of my career I wanted to 
    capitalize on my future endeavors. I put my faith 
    in my lord and savior Jesus Christ and was lead to 
    Slash to help further my career. Charles and Bugra 
    have always been open minded, honest, patient and 
    prescient.  They are truly for the player while 
    conducting business. They never question my worth 
    - they only help people value me and they stood on 
    businesses every time. Very thankful and grateful 
    to be a part of Slash Sports."`,
    image: "/images/coty-clarke.jpeg",
    alt: "Photo of SLASH basketball athlete Coty Clarke"
  },
  {
    id: 7,
    firstName: "Patrick",
    lastName: "Miller",
    quote: `"Ever since I first talked to Charles I 
    knew he was the best agent for me! He’s more than 
    an agent to me though, he’s a friend/brother! 
    Making the decision to change to Charles a few 
    years ago was the best decision I can make! Wish he 
    could have been my agent coming out of college! 
    Everything he said that would happen, happened 
    hahaha! And is continuing to happen! I trust him 
    and he is always honest rather it’s good or bad! I 
    feel like he is the best agent out there! I believe 
    that so much that I try to get all my homies that 
    hoop to sign with him! I’m thankful that he 
    represents me and I’m looking forward to what the 
    future holds for us! Appreciate you Charles!"`,
    image: "/images/patrick-miller.png",
    alt: "Photo of SLASH basketball athlete Patrick Miller"
  },
  {
    id: 8,
    firstName: "Johnny",
    lastName: "O'Bryant",
    quote: `"I signed with Slash at a pivotal point in 
    my career, and they were instrumental in helping me 
    make an important leap from one market to another. 
    The team has consistently had my back through every 
    challenge, big or small, and having an agency that 
    picks up the phone any time you call is vital for a 
    player overseas. I couldn’t have asked for a better 
    agency to represent me."`,
    image: "/images/johnny-obryant.jpeg",
    alt: "Photo of SLASH basketball athlete Johnny O'Bryant"
  },
  {
    id: 9,
    firstName: "Ray",
    lastName: "Parks Jr.",
    quote: `"Joining Slash Sports was without a doubt 
    the best decision I’ve ever made for my basketball 
    career. From the very beginning, Charles and Bugra 
    showed me what it truly means to work with people 
    who are genuine, honest, and deeply committed to my 
    success both on and off the court.//n//n
    
    They gave me a breath of fresh air at a time when I 
    needed it most. They believed in me, supported me, 
    and most importantly, took the time to truly 
    understand my situation. With them, I never felt 
    like just another client — I felt valued, respected, 
    and cared for.//n//n
    
    I can confidently say that with Charles and Bugra, 
    I am more than just an athlete they represent; I am 
    family.//n//n
    
    I am truly blessed to be a part of Slash Sports. 
    This partnership has not only helped me grow in my 
    career but also given me the confidence and peace 
    of mind to keep striving to be the best version of 
    myself."`,
    image: "/images/ray-parks.jpeg",
    alt: "Photo of SLASH basketball athlete Ray Parks Jr."
  },
  {
    id: 10,
    firstName: "Lisa",
    lastName: "Green",
    quote: `"Charles was my son’s agent, though I felt 
    I was his client because he always treated me as 
    such. He was professional, forthright, and never 
    over-sold a contract. He was always hands-on, even 
    to the extended family, which I truly appreciated. 
    He was available for any question and provided real 
    feedback. He is authentic, honest, and trustworthy. 
    Even though my son has since retired, we will always 
    be connected because of the relationship he built 
    with us."`,
    image: "/images/lisa-green.jpeg",
    alt: "Photo of Lisa Green mother of Former SLASH athlete"
  },
  {
    id: 11,
    firstName: "Pierre",
    lastName: "Jackson",
    quote: `"Ever since I made the decision to look for 
    new representation as far as agents Charles was my 
    choice right from the jump. Since my transition to 
    Charles it’s been great. At a stand still in my 
    career he has definitely helped get things back on 
    the right track and I couldn’t be more grateful for 
    his dedication to make sure things are handled the 
    right way and with no shady business. You are beyond 
    appreciated. Thank you Charles!"`,
    image: "/images/pierre-jackson.JPG",
    alt: "Photo of SLASH basketball athlete Pierre Jackson"
  },
  {
    id: 12,
    firstName: "Marvelle",
    lastName: "Harris",
    quote: `"Charles changed my life and career. He 
    came into my life at a rough time on and off the 
    court. He not only gets things handled on the 
    business side but we also became family. From 
    pulling up on me overseas to coming to the crib 
    every summer. He’s a stand up guy who’s always 
    going to do what’s right by you. He always ASK what 
    you think and makes sure you are comfortable about 
    the situation. Nothing is forced. We been building 
    for years now and it just keeps getting better 
    every year. Appreciate this guy because I honestly 
    don’t know where I’d be without him! #SLASH"`,
    image: "/images/marvelle-harris.png",
    alt: "Photo of SLASH basketball athlete Marvelle Harris"
  },
  {
    id: 13,
    firstName: "James",
    lastName: "McAdoo",
    quote: `"My career was at a crossroads one that I 
    financially and career wise could not afford to 
    mess up. That’s when my family and I decided to put 
    our trust in Bugra and Charles at SLASH. They took 
    control and got my career where it needed to be. 
    Keeping me informed and strongly represented all 
    along the way."`,
    image: "/images/james-mcadoo.png",
    alt: "Photo of SLASH basketball athlete James McAdoo"
  },
  {
    id: 14,
    firstName: "Lauren",
    lastName: "McAdoo",
    quote: `"Choosing an agent is one of the hardest 
    decisions you make as a professional athlete. I 
    could not be more thankful that our family was 
    introduced to Bugra and Charles. Over the years 
    they have worked tirelessly to ensure that my 
    husband’s (and our family’s) best interests are 
    always at the forefront of every decision. It truly 
    feels like they care about their players as 
    individuals, and have become a part of our family 
    along the way."`,
    image: "/images/lauren-mcadoo.jpeg",
    alt: "Photo of Lauren McAdoo wife of SLASH athlete James McAdoo"
  },
  {
    id: 15,
    firstName: "Marcus",
    lastName: "Ginyard",
    quote: `"I was lucky to have an 11 year professional 
    career, playing in many of the top national and 
    international leagues. Without a doubt, the best 
    thing that happened to me was signing with Slash. 
    Charles and Bugra put me in a position to play at 
    the highest level and supported me as a player AND 
    a person in a way that no other agent ever did. I 
    am forever grateful to them and wish they were my 
    agents from the beginning!"`,
    image: "/images/marcus-ginyard.jpeg",
    alt: "Photo of SLASH basketball athlete Marcus Ginyard"
  },
]